...
module.exports = (function() {
    ...
    create: function(connectionName, collectionName, data, cb) {
      spawnConnection(connectionName, function __CREATE__(connection, cb) {
        ...
        connection.query(query, function(err, result) {
          ...
          var key = getPrimayKeyData(collection);
          if (key.autoIncrement) {
            model.primaryKey = result.insertId;
          }
      ...
      spawnConnection(connectionName, function __CREATE_EACH__(connection, cb) {
        ...
        async.eachSeries(valuesList, function (data, cb) {
          ...
          connection.query(query, function(err, results) {
            if (err) return cb(err);
            var id = key.autoIncrement ? results.insertId : results[primaryKey];
            records.push(id);
            cb();
          });
        }, function(err) {
          ...
          var query = 'SELECT * FROM ' + tableName + ' WHERE ' + key.primaryKey + ' IN (' + records + ');';
  ...
  /**
   * Get data about the primary key for a given collection.
   *
   * @param {Object} collection
   */
  function getPrimayKeyData(collection) {
    var pk, auto, autoPK;
    // There doesn't appear to be a great way to get "autoPK" from the definition
    // so we resort to determining if id exists in the schema but wasn't specifically declared.
    autoPK = !collection.definition.hasOwnProperty('id') && collection.schema.hasOwnProperty('id');
    Object.keys(collection.definition).forEach(function(key) {
      if(collection.definition[key].primaryKey !== true) return;
      pk = key;
    });
    auto = !!collection.definition[pk].autoIncrement;
    return {
      primaryKey: pk,
      autoIncrement: auto,
      autoPK: autoPK
